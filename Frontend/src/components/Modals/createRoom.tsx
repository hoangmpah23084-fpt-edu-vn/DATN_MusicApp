...
const ModalCreateRoom = ({ onShowModal }: IProps) => {
    ...
    const onHandleSubmit: SubmitHandler<{ password: string , name: string }> = (data) => {
        console.log(data);
        reset()
    }
                ...
                <div className="fixed flex justify-center items-center z-50 w-full p-4 overflow-x-hidden overflow-y-auto  md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div className="relative w-full max-w-md max-h-full">
                        <div ref={ref} className="relative bg-[#231b2e] rounded-lg shadow dark:bg-gray-700 ">
                            <button onClick={() => onShowModal()} type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" >
                                <span><AiOutlineClose /></span>
                            </button>
                            <div className="px-6 py-6 lg:px-8">
                                <h3 className="mb-4 text-xl font-medium text-white dark:text-white">Tạo phòng</h3>
                                <form className="space-y-6" onSubmit={handleSubmit(onHandleSubmit)}>
                                    <div>
                                        <label className="block mb-2 text-sm font-medium text-white">Tên phòng</label>
                                        <input type="text" {...register("name")} placeholder="Tên phòng" className="bg-[#3a3244] text-white text-sm rounded-lg block w-full p-2.5" />
